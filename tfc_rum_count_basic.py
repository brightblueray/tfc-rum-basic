...
# set the base url
base_url = (
    os.environ.get("TF_ADDR") or f"{args.addr}/api/v2"
)  # ENV Variable overrides commandline
logging.info(f"Using Base URL: {base_url}")
server = urlparse(
    base_url
).netloc  # Need the server to parse the token from helper file
...
for o in organizations:
    ...
    if verbose:
        print(
            f"{'WS ID':24}{'Name':40}{'Resources':10}{'Version':9}{'Last-Updated':13}"
        )
    for ws in workspaces:
        if verbose:
            print(
                f"{ws['id']:24}{ws['attributes']['name']:40}{ws['attributes']['resource-count']:<10}{ws['attributes']['terraform-version']:9}{datetime.datetime.strptime(ws['attributes']['latest-change-at'], '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%Y-%m-%d'):13}"
            )
    ...
    for rs in resources:
        if (
            rs["attributes"]["provider-type"] == "null_resource"
            or rs["attributes"]["provider-type"] == "terraform_data"
        ):
            null_rs += 1
